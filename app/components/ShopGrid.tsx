    ...
    <div>
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap gap-3">
          <Select
            ...
            options={[{ label: "Category", value: "All" }, ...SERUM_TYPES.map((t) => ({ label: t, value: t }))]}
      ...
      {error && (
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-6 text-sm text-rose-700">
          Couldn&apos;t load products: {error}. Make sure the backend is running
          at <code>{process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000"}</code>.
        </div>
      )}
      ...
      {!isPending && !error && filtered.length === 0 && (
        <p className="py-16 text-center text-foreground/50">No serums found.</p>
      )}
